        ...
        (
            ...
            ru.make_side_by_side(
                ...
                ui.input_numeric(
                    "colsample_bytree",
                    "Column sample:",
                    value=self.state.get("colsample_bytree", 1.0),
                    min=0,
                    max=1,
                    step=0.1,
                ),
...
class model_xgboost:
    ...
    def shiny_ui(self, request: StarletteRequest):
        return ui.page_navbar(
            ...
            self.navbar,
            ru.ui_help(
                "https://github.com/vnijs/pyrsm/blob/main/examples/model/model-xgboost-classification.ipynb",
    ...
    def shiny_server(self, input: Inputs, output: Outputs, session: Session):
        ...
        mu.make_summary(
            ...
            estimate,
            ret="xgb",
            sum_fun=rsm.model.xgboost.summary,
        )
        mu.make_predict(
            self,
            input,
            ...
            estimate,
            ret="xgb",
            pred_fun=rsm.model.xgboost.predict,
            show_ci=False,
        )
        mu.make_plot(
            self,
...
def xgboost(
    ...
    rc = model_xgboost(data_dct, descriptions_dct, state=state, code=code)
    nest_asyncio.apply()
...
if __name__ == "__main__":
    ...
    # xgboost({"titanic": titanic}, {"titanic": titanic_description}, code=True)
    xgboost(debug=True)
